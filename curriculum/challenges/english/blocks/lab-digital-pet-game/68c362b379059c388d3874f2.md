...
```js
console.warn(document.documentElement.outerHTML);
...
```js
assert.exists(document.querySelector('#pet-name'));
```

This form should contain a `button`.

```js
assert.exists(document.querySelector('form')?.querySelector('button'));
```

When the input is given a value and the button is pressed, the form should no longer be visible.

```js
assert.doesNotExist(document.querySelector('form'));
```
